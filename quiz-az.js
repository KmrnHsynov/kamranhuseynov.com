...
function refreshPublishedTopics(cb){
  ...
  db.ref('student_quiz').once('value').then(snap=>{
    ...
    Object.entries(val).forEach(([key,exam])=>{
      if(exam&&exam.topic) publishedTopics[exam.topic]={ count: exam.questions?.length||0, key };
    });
    cb && cb();
  }).catch(()=>{ cb && cb(); });
}
